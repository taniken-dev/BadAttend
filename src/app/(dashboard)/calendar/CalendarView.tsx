  ...
  return (
    <div className="flex flex-col gap-4">
      ...
      <div className="flex flex-col gap-1.5">
        ...
        {/* バッジ行（全ロール共通） */}
        {(session.is_results_confirmed || session.is_cancelled) && (
              ...
              <div className="flex items-center gap-1.5 flex-wrap">
                ...
                {canManageSessions && (
                  <button
                    ...
                    className="flex items-center gap-1 text-xs px-2 py-0.5 rounded-full font-semibold cursor-pointer transition-opacity hover:opacity-70"
                    style={{ background: 'var(--gray-100)', color: 'var(--gray-500)', border: '1px solid var(--gray-200)', opacity: cancelSubmitting ? 0.6 : 1 }}
                  >
                    {cancelSubmitting
                      ? <span className="w-2.5 h-2.5 border border-current border-t-transparent rounded-full animate-spin" />
                      : <RotateCcw size={10} />}
                    {cancelSubmitting ? '処理中...' : '解除'}
                  </button>
                )}
              </div>
            )}
        ...
        {/* 休止にする（マネージャーのみ・非休止時・小さなアクション） */}
        {canManageSessions && !session.is_cancelled && (
          <div className="mt-1">
            {!cancelFormOpen ? (
              <button
                type="button"
